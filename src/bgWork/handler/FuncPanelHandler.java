...
public class FuncPanelHandler extends PanelHandler {
	...
	void init() {
		...
		while (this.getIcon(index) != null) {
			...
			btn.setBounds(boundDistance, boundDistance + (btnSize.height + boundDistance) * index, btnSize.width,
					btnSize.height);
			btn.setVisible(true);
	...
	void initContextPanel() {
		...
		contextPanel.setBounds(boundDistance, boundDistance, btnSize.width + boundDistance * 2,
				(btnSize.height + boundDistance) * btnlist.size() + boundDistance);
